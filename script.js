...
/* === LEXVOID HOTFIX REAL: inventário não some + insígnia fixa + comprado fixo === */
(function(){
  const $=(s,r=document)=>r.querySelector(s);
  const $$=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const safe=v=>String(v||'').trim().replace(/"/g,'%22').replace(/'/g,'%27');
  const norm=v=>String(v||'').trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/^(frame|moldura|effect|efeito|banner-effect|selo|seal|badge|insignia|insignia):/,'');
  const toastx=m=>{try{toast(m)}catch(e){console.log(m)}};
  const save=async m=>{try{if(typeof saveUser==='function') await saveUser(m||'Salvo!')}catch(e){console.warn(e)}};
  const inv=()=>{ if(!window.user) window.user={}; if(!Array.isArray(user.inventory)) user.inventory=[]; return user.inventory; };
  const src=o=>o?.url||o?.value||o?.img||o?.image||o?.imageUrl||o?.gif||'';
  function typeOf(it){ const t=norm(it?.type); if(t.includes('frame')||t.includes('moldura'))return'frame'; if(t.includes('effect')||t.includes('efeito')||t.includes('banner'))return'effect'; if(t.includes('selo')||t.includes('seal'))return'selo'; if(t.includes('badge')||t.includes('insign'))return'insignia'; return t||'other'; }
  function keys(o){ return [o?.id,o?.itemId,o?.frameId,o?.effectId,o?.seloId,o?.insigniaId,o?.url,o?.value,o?.name].map(norm).filter(Boolean).flatMap(x=>[x,x.replace(/^(frame|moldura|effect|efeito|selo|seal|badge|insignia):/,'')]); }
  function same(a,b){ const A=keys(a), B=keys(b); return A.some(x=>B.includes(x)); }
  function admins(kind){ const A=window.__LV_ADMIN_FINAL||{}; if(kind==='frame') return A.frames||window.customFrames||[]; if(kind==='effect') return A.effects||window.__lexAdminEffectsShop||[]; if(kind==='selo') return A.selos||window.__lexAdminSelosShop||[]; if(kind==='insignia') return A.insignias||window.__lexAdminInsigniasShop||[]; return []; }
  function adminMatch(it){ const k=typeOf(it); return admins(k).find(a=>same(it,a)||norm(a.id)===norm(it.id)||norm(a.name)===norm(it.name)); }
  function hydrate(it){ const a=adminMatch(it); return a?{...a,...it,type:typeOf(it),url:src(it)||src(a),value:src(it)||src(a),name:it.name||a.name,desc:it.desc||a.desc,prices:it.prices||a.prices,price:it.price||a.price}:it; }
  function isOwned(kind,item){ return inv().map(hydrate).some(it=>typeOf(it)===kind && same(it,item)); }
  function getInvFilter(){ try{return window.inventoryFilter||inventoryFilter}catch(e){return $('#inventoryTabs button.active')?.dataset.invFilter||'all'} }
  function setInvFilter(v){ try{window.inventoryFilter=v; inventoryFilter=v}catch(e){window.inventoryFilter=v} }
  function pass(it){ const f=norm(getInvFilter()), t=typeOf(it); if(['all','todos','tudo'].includes(f))return true; if(['molduras','frames','frame'].includes(f))return t==='frame'; if(['efeitos','effects','effect'].includes(f))return t==='effect'; if(['selos','selo'].includes(f))return t==='selo'; if(['insignias','insignia','badges','badge'].includes(f))return t==='insignia'; if(['presentes','gifts'].includes(f))return !!it.gift; return true; }
  function avatar(){try{return getBestAvatar()||user?.avatar||''}catch(e){return user?.avatar||''}}
  function preview(it){ it=hydrate(it); const k=typeOf(it), u=src(it); if(k==='frame') return `<div class="lex-shop-frame-preview lex-inv-frame-preview"><span class="lex-shop-avatar" style="background-image:url('${safe(avatar())}')"></span>${u?`<img src="${safe(u)}" onerror="this.style.display='none'">`:''}</div>`; if(k==='effect') return `<div class="asset-preview lex-inv-effect-preview" style="background-image:url('${safe(u)}')"></div>`; if(k==='selo'||k==='insignia') return `<div class="asset-preview lex-inv-selo-preview">${u?`<img src="${safe(u)}" onerror="this.style.display='none'">`:'🏷️'}</div>`; return '<div class="asset-preview">✦</div>'; }
  function ensureTabs(){ const invt=$('#inventoryTabs'); if(invt && !invt.querySelector('[data-inv-filter="insignias"]')){ const b=document.createElement('button'); b.type='button'; b.dataset.invFilter='insignias'; b.textContent='Insígnias'; const eff=invt.querySelector('[data-inv-filter="effects"],[data-inv-filter="efeitos"]'); (eff||invt.lastElementChild).insertAdjacentElement(eff?'beforebegin':'afterend',b); } }
  function renderInventorySafe(){ ensureTabs(); const grid=$('#inventoryGrid'); if(!grid) return; const filter=getInvFilter(); $$('#inventoryTabs button').forEach(b=>b.classList.toggle('active',b.dataset.invFilter===filter)); const coin=$('#invCoins'); if(coin) coin.textContent=Number(user?.coins||0); const cnt=$('#invItemsCount'); if(cnt) cnt.textContent=inv().length; const items=inv().map((raw,i)=>({raw:hydrate(raw),i})).filter(x=>pass(x.raw)); if(!items.length){ grid.innerHTML='<p>Nenhum item nessa categoria.</p>'; return; } grid.innerHTML=items.map(({raw:it,i})=>{ const k=typeOf(it); return `<div class="asset-card inv-item-card lex-inv-card" data-lvx-type="${esc(k)}" data-lvx-index="${i}"><div class="lex-inv-preview-wrap">${preview(it)}</div><div class="asset-body"><b>${esc(it.name||'Item')}</b><small>${esc(k)}</small>${k==='frame'?`<button class="btn primary small" type="button" data-use-inv-frame="${i}">Usar</button><button class="btn dark small" type="button" data-adjust-inv-frame="${i}">Ajustar</button>`:''}${k==='effect'?`<button class="btn primary small" type="button" data-lvx-use="${i}">Usar</button>`:''}${k==='selo'?`<button class="btn primary small" type="button" data-lvx-use="${i}">Usar</button><button class="btn dark small" type="button" data-lvx-adjust="${i}">Ajustar</button>`:''}${k==='insignia'?`<button class="btn primary small" type="button" data-lvx-use="${i}">Usar</button><button class="btn dark small" type="button" data-lvx-adjust="${i}">Ajustar</button>`:''}<button class="btn dark small" type="button" data-lvx-remove-profile="${i}">Remover do perfil</button></div></div>`; }).join(''); }
  window.renderInventory=renderInventorySafe; try{renderInventory=window.renderInventory}catch(e){}

  function renderInsignias(){ const slug=$('#profileSlug2')||$('#profileSlug')||$('.profile-slug')||$('#profileUsername')||$('.profile-username'); if(!slug||!window.user)return; let box=$('#profileInsignias'); if(!box){ box=document.createElement('div'); box.id='profileInsignias'; box.className='profile-insignias'; } slug.insertAdjacentElement('afterend',box); const arr=Array.isArray(user.insignias)?user.insignias:[]; box.innerHTML=arr.map(i=>{const u=src(i); return u?`<img src="${safe(u)}" title="${esc(i.name||'Insígnia')}" style="width:${Number(i.size||34)}px;height:${Number(i.size||34)}px;object-fit:contain">`:''}).join(''); box.style.display=arr.length?'flex':'none'; }
  function applyBannerEffect(){ const banner=$('#profileBanner'); if(!banner)return; const u=user?.bannerEffect||''; banner.style.setProperty('--banner-effect-url',`url('${safe(u)}')`); banner.classList.toggle('has-banner-effect',!!u); let fx=banner.querySelector('.lex-banner-effect-layer'); if(!fx&&u){fx=document.createElement('span');fx.className='lex-banner-effect-layer';banner.appendChild(fx)} if(fx){fx.style.backgroundImage=u?`url('${safe(u)}')`:'';fx.style.display=u?'block':'none'} }
  async function useItem(i){ const it=hydrate(inv()[Number(i)]); if(!it)return; const k=typeOf(it), u=src(it); if(k==='effect'){ user.bannerEffect=u; await save('Efeito aplicado!'); applyBannerEffect(); }
    else if(k==='selo'){ user.selos=Array.isArray(user.selos)?user.selos:[]; if(!user.selos.some(x=>same(x,it))) user.selos.push({id:it.id,itemId:it.itemId,name:it.name,type:'selo',url:u,value:u,size:Number(it.size||28)}); await save('Selo aplicado!'); }
    else if(k==='insignia'){ user.insignias=Array.isArray(user.insignias)?user.insignias:[]; if(!user.insignias.some(x=>same(x,it))) user.insignias.push({id:it.id,itemId:it.itemId,name:it.name,type:'insignia',url:u,value:u,size:Number(it.size||34)}); await save('Insígnia aplicada!'); renderInsignias(); }
    try{window.renderProfile&&window.renderProfile()}catch(e){} renderInventorySafe(); toastx(k==='insignia'?'Insígnia aplicada!':'Item aplicado!'); }
  async function removeProfile(i){ const it=hydrate(inv()[Number(i)]); if(!it)return; const k=typeOf(it), u=norm(src(it)); if(k==='frame'){ user.frame=''; user.frameUrl=''; user.activeFrame=''; user.frameAdjust={x:0,y:0,scale:1,rotate:0}; $('#profileFrame')?.removeAttribute('src'); const pf=$('#profileFrame'); if(pf) pf.style.display='none'; await save('Moldura removida só do perfil.'); }
    if(k==='effect'){ if(norm(user.bannerEffect)===u) user.bannerEffect=''; await save('Efeito removido só do perfil.'); applyBannerEffect(); }
    if(k==='selo'){ user.selos=(user.selos||[]).filter(x=>!same(x,it)); await save('Selo removido só do perfil.'); }
    if(k==='insignia'){ user.insignias=(user.insignias||[]).filter(x=>!same(x,it)); await save('Insígnia removida só do perfil.'); renderInsignias(); }
    try{window.renderProfile&&window.renderProfile()}catch(e){} renderInventorySafe(); }
  function adjustSmall(i){ const it=hydrate(inv()[Number(i)]); const k=typeOf(it), u=src(it); if(!it||!u)return; const size=Number(it.size|| (k==='insignia'?34:28)); $('#lvxAdjustMini')?.remove(); document.body.insertAdjacentHTML('beforeend',`<div id="lvxAdjustMini" class="modal show"><div class="modal-card lex-selo-adjust-card"><button class="modal-close" id="lvxAdjClose">×</button><h2>Ajustar ${k==='insignia'?'insígnia':'selo'}</h2><p>Controle o tamanho no perfil.</p><div class="lex-selo-adjust-preview"><span>@usuario</span><img src="${safe(u)}" style="width:${size}px;height:${size}px;object-fit:contain"></div><label>Tamanho <input id="lvxSize" type="range" min="12" max="90" value="${size}"></label><button class="btn primary" id="lvxAdjSave">Salvar ajuste</button></div></div>`); const modal=$('#lvxAdjustMini'),img=modal.querySelector('img'),r=$('#lvxSize'); r.oninput=()=>{img.style.width=r.value+'px'; img.style.height=r.value+'px'}; $('#lvxAdjClose').onclick=()=>modal.remove(); $('#lvxAdjSave').onclick=async()=>{ it.size=Number(r.value); inv()[Number(i)]={...inv()[Number(i)],size:Number(r.value)}; const arr=k==='insignia'?'insignias':'selos'; user[arr]=(user[arr]||[]).map(x=>same(x,it)?{...x,size:Number(r.value)}:x); await save('Ajuste salvo!'); modal.remove(); renderInsignias(); try{window.renderProfile&&window.renderProfile()}catch(e){} renderInventorySafe(); }; }
  function patchShopOwned(){ $$('.zyo-item-card[data-lvf-kind]').forEach(card=>{ const kind=card.dataset.lvfKind, id=card.dataset.lvfId; const item=admins(kind).find(a=>String(a.id||a.url||a.name)===String(id)||norm(a.id)===norm(id)||norm(a.name)===norm(id)); if(!item)return; if(isOwned(kind,item)){ card.classList.add('owned'); const btn=card.querySelector('[data-lvf-buy],[data-lv-open-buy]'); if(btn){btn.disabled=true; btn.textContent='🔒 Já comprado';} if(!card.querySelector('.owned-badge')) card.querySelector('.zyo-item-top div, .zyo-item-top')?.insertAdjacentHTML('beforeend','<span class="owned-badge">✓ Já comprado</span>'); } }); }
  const oldShop=window.renderShop; window.renderShop=function(){ const r=oldShop&&oldShop.apply(this,arguments); setTimeout(patchShopOwned,80); return r; }; try{renderShop=window.renderShop}catch(e){}
  document.addEventListener('click',function(e){ const tab=e.target.closest('#inventoryTabs [data-inv-filter]'); if(tab){ setInvFilter(tab.dataset.invFilter); setTimeout(renderInventorySafe,0); }
    const u=e.target.closest('[data-lvx-use]'); if(u){ e.preventDefault(); e.stopImmediatePropagation(); return useItem(u.dataset.lvxUse); }
    const rm=e.target.closest('[data-lvx-remove-profile]'); if(rm){ e.preventDefault(); e.stopImmediatePropagation(); return removeProfile(rm.dataset.lvxRemoveProfile); }
    const ad=e.target.closest('[data-lvx-adjust]'); if(ad){ e.preventDefault(); e.stopImmediatePropagation(); return adjustSmall(ad.dataset.lvxAdjust); }
    const buy=e.target.closest('[data-lvf-buy],[data-lv-open-buy]'); if(buy){ const card=buy.closest('.zyo-item-card'); const kind=buy.dataset.lvfBuy||buy.dataset.lvOpenBuy||card?.dataset.lvfKind; const id=buy.dataset.lvfId||buy.dataset.lvOpenId||card?.dataset.lvfId; const item=admins(kind).find(a=>String(a.id||a.url||a.name)===String(id)||norm(a.id)===norm(id)||norm(a.name)===norm(id)); if(item && isOwned(kind,item)){ e.preventDefault(); e.stopImmediatePropagation(); patchShopOwned(); return toastx('Você já comprou esse item.'); }}
  }, true);
  const oldProfile=window.renderProfile; window.renderProfile=function(){ if(oldProfile) oldProfile.apply(this,arguments); setTimeout(()=>{renderInsignias(); applyBannerEffect();},30); }; try{renderProfile=window.renderProfile}catch(e){}
  document.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{renderInventorySafe(); patchShopOwned(); renderInsignias();},1000));
  setInterval(()=>{ if($('#inventoryGrid')) renderInventorySafe(); patchShopOwned(); renderInsignias(); },2500);
})();

/* ===== LEXVOID HOTFIX — esconder moldura quebrada quando não está usando nenhuma ===== */
(function(){
  const $ = (s,r=document)=>r.querySelector(s);
  function realFrameUrl(){
    try{
      const u = window.user || {};
      return String(u.frame || u.frameUrl || u.activeFrame || '').trim();
    }catch(e){ return ''; }
  }
  function hideBrokenFrameNow(){
    try{
      const dec = $('#avatarDecoration');
      const frameUrl = realFrameUrl();
      const hasFrame = !!frameUrl && !/^null$|^undefined$|^about:blank$/i.test(frameUrl);
      const pf = $('#profileFrame');
      if(dec){
        dec.classList.toggle('has-img-frame', hasFrame);
        dec.classList.toggle('lex-no-frame', !hasFrame);
      }
      if(pf){
        pf.onerror = function(){
          this.classList.add('lex-frame-broken','dlinky-frame-broken','dlinky-frame-off');
          this.style.setProperty('display','none','important');
          this.style.setProperty('visibility','hidden','important');
          this.removeAttribute('src');
        };
        if(!hasFrame){
          pf.classList.add('lex-frame-broken','dlinky-frame-broken','dlinky-frame-off');
          ...
          pf.classList.remove('lex-frame-broken','dlinky-frame-broken','dlinky-frame-off','dlinky-frame-hidden');
          if(pf.getAttribute('src') !== frameUrl) pf.src = frameUrl;
          pf.style.setProperty('display','block','important');
          pf.style.setProperty('visibility','visible','important');
          pf.style.setProperty('opacity','1','important');
        }
      }
      if(dec && !hasFrame){
        dec.querySelectorAll('img:not(#profileAvatar)').forEach(img=>{
          img.classList.add('lex-frame-broken');
  ...
  window.renderProfile = function(){
    ...
    hideBrokenFrameNow();
    setTimeout(hideBrokenFrameNow, 80);
    setTimeout(hideBrokenFrameNow, 350);
  };
  try{ if(typeof renderProfile !== 'undefined') renderProfile = window.renderProfile; }catch(e){}
  document.addEventListener('DOMContentLoaded', hideBrokenFrameNow);
